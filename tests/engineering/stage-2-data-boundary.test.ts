...
const forbidden = /(?:cambridge|ielts|cet4|cet6|wordlist|corpus|answer[-_ ]?key|vocabulary[-_ ]?dataset)/i;
function files(dir: string): string[] { return readdirSync(dir, { withFileTypes: true }).flatMap(e => { const p=path.join(dir,e.name); return e.isDirectory() ? files(p) : [p]; }); }
...
describe('Stage 2 lawful data boundary', () => {
  ...
  it('keeps canonical English data policy documented', async () => {
    const text = await import('node:fs/promises').then(fs => fs.readFile('docs/governance/stage-2-data-boundary.md', 'utf8'));
