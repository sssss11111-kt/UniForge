...
describe('LangGraph checkpoint recovery', () => {
  it('retains a versioned runtime checkpoint and rejects incompatible versions', async () => {
    ...
    const context = { actorId: 'actor' as never, workspaceId: 'workspace' as never, correlationId: 'recovery' };
    ...
    const checkpoint = store.values()[0]; expect(checkpoint).toBeDefined(); if (!checkpoint) return; expect(checkpoint).toMatchObject({ schemaVersion: 1, adapterVersion: '0.1.0', runId: run.value.id, cursor: 'approval' });
    await store.save({ ...checkpoint, adapterVersion: '0.0.0' });
    expect(await adapter.execute(context, run.value.id)).toMatchObject({ ok: false, error: { code: 'UNAVAILABLE' } });
  });
});
