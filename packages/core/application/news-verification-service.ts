...
export class NewsVerificationService {
  ...
  verify(input: {
    ...
    if (
      input.supportingSourceIds.some((id) => !id.trim()) ||
      input.conflictingSourceIds.some((id) => !id.trim())
    )
  ...
  correct(input: { correction: NewsCorrection; permissions: readonly string[] }): NewsCorrection {
    ...
    if (!correction.id.trim() || !correction.claimId.trim())
      throw new Error('Correction id and claim id are required');
    ...
    if (correction.sourceIds.some((id) => !id.trim()))
      throw new Error('Correction source required');
    ...
    if (history.some((item) => item.id === correction.id))
    ...
    const c = this.cloneCorrection({
      ...
      ...(correction.evidence
        ? { evidence: correction.evidence.map((item) => ({ ...item })) }
        : {}),
  ...
  recordCorrectionAttempt(input: {
    ...
    const c = this.cloneCorrection({
      ...input.correction,
      outcome: input.correction.outcome ?? 'PENDING_APPROVAL',
    });
