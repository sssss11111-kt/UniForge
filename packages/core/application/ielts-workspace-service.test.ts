...
describe('IeltsWorkspaceService', () => {
  it('returns the complete navigation without bundled restricted content', () => {
    const snapshot = new IeltsWorkspaceService().getSnapshot({
      actor: 'user',
      permissions: ['english:read'],
    });
  ...
  it('fails closed without permission', () => {
    expect(() =>
      new IeltsWorkspaceService().getSnapshot({ actor: 'user', permissions: [] }),
    ).toThrow('english:read');
  });
});
