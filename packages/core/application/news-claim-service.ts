import type {
  CreateNewsClaimInput,
  NewsClaim,
  UpdateNewsClaimStatusInput,
...
export class NewsClaimService {
  ...
  create(input: CreateNewsClaimInput): NewsClaim {
    ...
    if (input.claim.confidence < 0 || input.claim.confidence > 1)
      throw new Error('Confidence must be between 0 and 1');
    const c = {
      ...input.claim,
      evidence: input.claim.evidence.map((x) => ({ ...x })),
      provenance: {
        ...input.claim.provenance,
        sourceEventIds: [...input.claim.provenance.sourceEventIds],
      },
  ...
  verify(input: UpdateNewsClaimStatusInput): NewsClaim {
    ...
    if (c.provenance.origin === 'AI' && c.status === 'PENDING') {
      /* verification is an explicit write */
    }
    const a = { ...c, status: 'VERIFIED' as const };
    this.claims.set(c.id, a);
    return this.clone(a);
  }
  reject(input: UpdateNewsClaimStatusInput): NewsClaim {
    ...
    const c = this.claims.get(input.claimId);
    if (!c) throw new Error('Claim not found');
    const a = { ...c, status: 'REJECTED' as const };
    this.claims.set(c.id, a);
    return this.clone(a);
  }
  private clone(c: NewsClaim): NewsClaim {
    return {
      ...c,
      evidence: c.evidence.map((x) => ({ ...x })),
      provenance: { ...c.provenance, sourceEventIds: [...c.provenance.sourceEventIds] },
    };
  }
