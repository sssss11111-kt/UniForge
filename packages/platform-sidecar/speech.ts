...
export class SpeechSidecarPort {
  ...
  async health(): Promise<{ status: 'ready' | 'unavailable'; reason?: string }> {
    ...
    return value.status === 'ready'
      ? { status: 'ready' }
      : { status: 'unavailable', ...(value.reason ? { reason: value.reason } : {}) };
  }
  async transcribe(audio: VoiceAudio, signal?: AbortSignal): Promise<string> {
    const result = await this.host.request(
      ...
      {
        operation: 'STT',
        audio: { format: audio.format, base64: audio.base64 },
      } as unknown as Json,
      signal,
    );
  ...
  async synthesize(text: string, signal?: AbortSignal): Promise<VoiceAudio> {
    ...
    if (typeof value.format !== 'string' || typeof value.base64 !== 'string')
