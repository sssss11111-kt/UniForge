...
export class ObsidianAdapter {
  ...
  read(path: string, sourceId = path): Promise<Result<MarkdownSource>> {
    return readMarkdownSource(path, sourceId);
  }
  diff(source: MarkdownSource, content: string): MarkdownDiff {
    return createMarkdownDiff(source, content);
  }
