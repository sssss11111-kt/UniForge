...
export interface FoundationGraph {
  invoke(state: RuntimeState, options: { interruptBeforeWrite: boolean; signal?: AbortSignal }): Promise<RuntimeState>;
}
export const unavailableFoundationGraph = (): FoundationGraph => ({
  async invoke() { throw new Error('LANGGRAPH_ADAPTER_UNAVAILABLE'); },
});
