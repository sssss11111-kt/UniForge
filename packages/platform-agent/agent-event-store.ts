...
export class InMemoryAgentEventStore implements AgentEventStore {
  ...
  append(event: AgentEvent): void {
    ...
    if (previous && event.runSeq !== previous.runSeq + 1) throw new Error('AGENT_EVENT_SEQUENCE_CONFLICT');
    if (!previous && event.runSeq !== 1) throw new Error('AGENT_EVENT_SEQUENCE_CONFLICT');
    if (list.some((item) => item.eventId === event.eventId)) throw new Error('AGENT_EVENT_DUPLICATE');
    list.push(event); this.history.set(event.runId, list);
  }
  events(runId: Id<'agent-run'>, afterSeq = 0): AgentEvent[] {
    return (this.history.get(runId) ?? []).filter((event) => event.runSeq > afterSeq).map((event) => ({ ...event }));
  }
  all(runId: Id<'agent-run'>): AgentEvent[] { return this.events(runId, 0); }
  snapshot(runId: Id<'agent-run'>): AgentRun | undefined { const run = this.runs.get(runId); return run && { ...run }; }
  saveSnapshot(run: AgentRun): void { this.runs.set(run.id, { ...run }); }
}
