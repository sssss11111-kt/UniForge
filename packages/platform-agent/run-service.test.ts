...
const definition = { id: 'agent-definition-test' as Id<'agent-definition'>, version: 1, role: 'test', domain: 'test', modelPolicy: {}, contextPolicy: {}, toolPolicy: {}, permissionPolicy: {}, budgetPolicy: {}, outputSchema: {} } satisfies AgentDefinition;
const context = (workspaceId = 'workspace-a'): RequestContext => ({ actorId: 'actor-a' as Id<'actor'>, workspaceId: workspaceId as Id<'workspace'>, correlationId: `c-${workspaceId}` });
async function makeRun() { const service = new RunService(); const created = await service.createRun(context(), { taskId: 'task-a' as Id<'task'>, definition, runtime: 'fixture' }); expect(created.ok).toBe(true); if (!created.ok) throw new Error('create failed'); return { service, run: created.value }; }
...
describe('AgentRuntime lifecycle', () => {
  ...
  it('pauses for approval and refuses automatic continuation', async () => {
    const { service, run } = await makeRun(); await service.start(context(), run.id);
  ...
  it('supports replayable, paged streams and rejects cross-workspace inspection', async () => {
    const { service, run } = await makeRun(); await service.start(context(), run.id); await service.pause(context(), run.id, 'user pause');
    const events = []; for await (const item of service.stream(context(), run.id, 1)) events.push(item.event);
    expect(events.map((event) => event.runSeq)).toEqual([2, 3]);
    expect((await service.inspect(context('workspace-b'), run.id)).ok).toBe(false);
    const fork = await service.fork(context(), run.id); expect(fork.ok && fork.value.id).not.toBe(run.id);
  });
});
