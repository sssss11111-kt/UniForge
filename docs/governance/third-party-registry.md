...
| ts-fsrs                             |   5.4.2 | MIT        | https://github.com/open-spaced-repetition/ts-fsrs/blob/v5.4.2/LICENSE       | Pure TypeScript/JavaScript FSRS v6 adapter; Node >=20 and Windows supported; no native runtime | A        |
...
All listed packages are redistributed only through the development lockfile at this stage; no installer or bundled runtime artifact is shipped. The packages are MIT, Apache-2.0, or the explicitly recorded `@types/node` MIT license, with no additional sub-license identified in the upstream license files. Upstream projects publish source and security advisories through their official repositories and npm packages; dependency updates remain pinned and require a new registry review. Electron Forge's transitive archive and extraction advisories are recorded in the Task 0.1 evidence and remain a release gate. `ts-fsrs@5.4.2` is pinned in the core workspace for Stage 2 scheduling and is used only through `TsFsrsAdapter`; upgrading requires a new API, license, and security review.

## Stage 4 news connector and dataset admission

No production news connector or dataset is currently enabled. Runtime `RSS`, `WEB`, `CONNECTOR`, and `USER` observations are user-authorized proposals and are not bundled corpora. A connector may be enabled only after this registry records its pinned implementation/version, repository, data and content license (including redistribution terms), network and credential scope, retention/caching behavior, Windows/security review, and maintenance owner. Missing scope evidence or license evidence is a release-blocking violation.

Paywalled or private feeds, scraped article archives, publisher datasets, and any corpus with unclear licensing are prohibited from the application bundle. Credentials, cookies, bearer tokens, and API keys must remain in OS secure storage and must not appear in source, fixtures, logs, backups, or generated artifacts. The `news-boundary` gate scans production bundle roots (`apps/desktop/src`, `packages`, and `fixtures`) for restricted corpus/archive names and secret-shaped values.
